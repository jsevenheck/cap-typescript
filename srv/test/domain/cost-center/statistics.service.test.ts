import type { Transaction } from '@sap/cds';

import { getCostCenterStatistics } from '../../../domain/cost-center/services/statistics.service';

// Mock the cds module
jest.mock('@sap/cds', () => ({
  ql: {
    SELECT: {
      from: jest.fn().mockReturnThis(),
      columns: jest.fn().mockReturnThis(),
      where: jest.fn().mockReturnThis(),
    },
  },
}));

describe('CostCenterStatisticsService', () => {
  describe('getCostCenterStatistics', () => {
    interface MockTransaction {
      tx: Transaction;
      runFn: jest.Mock;
    }

    const createMockTransaction = (results: unknown[]): MockTransaction => {
      let callIndex = 0;
      const runFn = jest.fn().mockImplementation(() => {
        const result = results[callIndex] || [{ count: 0 }];
        callIndex++;
        return Promise.resolve(result);
      });
      return {
        tx: { run: runFn } as unknown as Transaction,
        runFn,
      };
    };

    it('should return statistics with all zeros when no cost centers exist', async () => {
      const emptyResults = Array(5).fill([{ count: 0 }]);
      const { tx } = createMockTransaction(emptyResults);

      const stats = await getCostCenterStatistics(tx);

      expect(stats.totalCostCenters).toBe(0);
      expect(stats.activeCostCenters).toBe(0);
      expect(stats.expiredCostCenters).toBe(0);
      expect(stats.upcomingExpiry).toBe(0);
      expect(stats.withAssignedEmployees).toBe(0);
    });

    it('should return correct statistics when cost centers exist', async () => {
      const results = [
        [{ count: 50 }], // total
        [{ count: 40 }], // active
        [{ count: 5 }],  // expired
        [{ count: 3 }],  // upcoming expiry
        [{ count: 30 }], // with assigned employees
      ];
      const { tx } = createMockTransaction(results);

      const stats = await getCostCenterStatistics(tx);

      expect(stats.totalCostCenters).toBe(50);
      expect(stats.activeCostCenters).toBe(40);
      expect(stats.expiredCostCenters).toBe(5);
      expect(stats.upcomingExpiry).toBe(3);
      expect(stats.withAssignedEmployees).toBe(30);
    });

    it('should pass clientId filter when provided', async () => {
      const results = Array(5).fill([{ count: 10 }]);
      const { tx, runFn } = createMockTransaction(results);
      const clientId = 'test-client-123';

      const stats = await getCostCenterStatistics(tx, clientId);

      // Verify that run was called 5 times (once for each statistic)
      expect(runFn).toHaveBeenCalledTimes(5);
      expect(stats.totalCostCenters).toBe(10);
    });

    it('should handle string count values', async () => {
      const results = [
        [{ count: '25' }], // string count
        [{ count: 15 }],
        [{ count: 10 }],
        [{ count: '8' }], // string count
        [{ count: 2 }],
      ];
      const { tx } = createMockTransaction(results);

      const stats = await getCostCenterStatistics(tx);

      expect(stats.totalCostCenters).toBe(25);
      expect(stats.upcomingExpiry).toBe(8);
    });

    it('should handle missing count values gracefully', async () => {
      const results = [
        [{}], // missing count
        [{ count: 15 }],
        [{ count: null }], // null count
        [{ count: undefined }], // undefined count
        [], // empty result
      ];
      const { tx } = createMockTransaction(results);

      const stats = await getCostCenterStatistics(tx);

      expect(stats.totalCostCenters).toBe(0);
      expect(stats.activeCostCenters).toBe(15);
      expect(stats.expiredCostCenters).toBe(0);
      expect(stats.upcomingExpiry).toBe(0);
      expect(stats.withAssignedEmployees).toBe(0);
    });

    it('should run all queries in parallel', async () => {
      const results = Array(5).fill([{ count: 1 }]);
      const { tx, runFn } = createMockTransaction(results);

      const startTime = Date.now();
      await getCostCenterStatistics(tx);
      const endTime = Date.now();

      // All 5 queries should be called
      expect(runFn).toHaveBeenCalledTimes(5);
      // Execution should be fast (parallel) - less than 100ms for mock calls
      expect(endTime - startTime).toBeLessThan(100);
    });

    it('should return all zeros for empty client scope array', async () => {
      const results = Array(5).fill([{ count: 10 }]);
      const { tx, runFn } = createMockTransaction(results);

      const stats = await getCostCenterStatistics(tx, []);

      // No queries should be executed for empty scope
      expect(runFn).not.toHaveBeenCalled();
      // All stats should be zero
      expect(stats.totalCostCenters).toBe(0);
      expect(stats.activeCostCenters).toBe(0);
      expect(stats.expiredCostCenters).toBe(0);
      expect(stats.upcomingExpiry).toBe(0);
      expect(stats.withAssignedEmployees).toBe(0);
    });
  });
});
